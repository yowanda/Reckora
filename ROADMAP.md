...
- **Phase 5 — Collaborative platform**: multi-user investigations, shared evidence library, role-based reporting. **Step 1** — per-dossier comments + assignment (`apps/api/reckora_api/collab/`, comment thread + assignee list with audit trail, gated by the existing access layer) — landed.
