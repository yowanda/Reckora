...
- **Phase 5 — Collaborative platform**: multi-user investigations, shared evidence library, role-based reporting. **Step 1** — per-dossier comments + assignment (`apps/api/reckora_api/collab/`, comment thread + assignee list with audit trail, gated by the existing access layer) — landed. **Step 3** — per-dossier activity feed (`apps/api/reckora_api/activity/`, `GET /api/v1/subjects/{id}/activity`; chronological newest-first projection over the existing comment / assignee / share / anchor tables, gated by the same read access as comments — owner / sharer / assignee / admin — with `actor` / `target` user resolution and a leading body excerpt for comment events) — landed.
