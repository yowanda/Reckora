...
| 10 | Web UI | partial | FastAPI backend with JWT auth at `/api/v1/*` (`apps/api/reckora_api`, `reckora-api serve`), two-tier RBAC (`admin` / `viewer`) with per-dossier ownership and explicit read-only sharing (`apps/api/reckora_api/access/`, `subject_owners` + `subject_shares` tables, `POST /api/v1/subjects/{id}/share`, `GET /api/v1/users`, `PATCH /api/v1/users/{id}/role`) — landed. Per-dossier comment thread + assignee list (`apps/api/reckora_api/collab/`, `subject_comments` + `subject_assignees` tables, `POST/GET/DELETE /api/v1/subjects/{id}/comments`, `POST/GET/DELETE /api/v1/subjects/{id}/assignees`; comments visible to anyone with read access — owner / sharer / assignee / admin — and removable by author / owner / admin; assignment mutations gated to owner / admin; assignment grants implicit read access; `assigned_by` audit trail with `ON DELETE SET NULL` so the row survives the granting user's deletion) — landed. Cross-dossier shared evidence library (`apps/api/reckora_api/xref/`, denormalised `subject_identifiers` index in the engine SQLite repo with idempotent backfill on open + Neo4j `(:Identifier)<-[:HAS_IDENTIFIER]-(:Subject)` parity, `GET /api/v1/subjects/{id}/cross-references` grouped by shared identifier, access-filtered against `subject_owners` / `subject_shares` so viewers only see matches they can read while admins see every match including legacy un-owned dossiers) — landed. Dashboard / graph viewer / report viewer (`apps/web/`, Vite + React + TS, stack confirmed when work starts) — pending user instruction. |
...
- **Phase 5 — Collaborative platform**: multi-user investigations, shared evidence library, role-based reporting. **Step 1** — per-dossier comments + assignment (`apps/api/reckora_api/collab/`, comment thread + assignee list with audit trail, gated by the existing access layer) — landed. **Step 2** — cross-dossier shared evidence library (`apps/api/reckora_api/xref/`, `GET /api/v1/subjects/{id}/cross-references` grouped by shared identifier, denormalised `subject_identifiers` index with idempotent backfill, Neo4j parity, access-filtered) — landed.
